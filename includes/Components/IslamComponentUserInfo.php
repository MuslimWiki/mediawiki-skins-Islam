...
/**
 * IslamComponentUserInfo component
 */
class IslamComponentUserInfo implements IslamComponent {
	...
	private function getUserEditCount(): ?array {
		...
		return [
			...
			'label' => $this->localizer->msg( 'islam-user-info-edits-label' )->text()
		];
	}
	...
	private function getUserRegistration(): ?array {
		...
		return [
			'value' => $html,
			'label' => $this->localizer->msg( 'islam-user-info-joined-label' )->text()
		];
	}
	...
	private function getUserGroups(): ?array {
		...
		foreach ( $groups as $group ) {
			...
			$listItem = new IslamComponentMenuListItem( $link, 'islam-userInfo-usergroup', $id );
	...
	public function getTemplateData(): array {
		...
		if ( $user->isRegistered() ) {
			...
			if ( $user->isTemp() ) {
				$data['text'] = $localizer->msg( 'islam-user-info-text-temp' );
			} else {
			...
			$data = [
				...
				'text' => $localizer->msg( 'islam-user-info-text-anon' )
			];
		}
