...
/**
 * IslamComponentPageHeading component
 * FIXME: Need unit test
 */
class IslamComponentPageHeading implements IslamComponent {
	...
	private function buildEditCountTagline( User $user ): string {
		...
		return Html::rawElement(
			'span',
			[
				'id' => 'islam-tagline-user-editcount',
	...
	private function buildRegistrationDateTagline( User $user ): string {
		...
		$regDateHtml = Html::rawElement(
			'time',
			[
				'class' => 'islam-user-regdate',
				'datetime' => $regDateTs,
			],
			$this->pageLang->userDate( new MWTimestamp( $regDate ), $user )
		);

		$msgRegDate = $this->localizer->msg( 'islam-tagline-user-regdate', $regDateHtml )->parse();

		return Html::rawElement(
			'span',
			[
				'id' => 'islam-tagline-user-regdate',
	...
	private function determineTagline(): string {
		...
		if ( $title->isTalkPage() ) {
			return $this->getIslamTagline( 'islam-tagline-ns-talk' );
		}
		...
		$nsMsgKey = 'islam-tagline-ns-' . strtolower( $title->getNsText() );
		if ( !$this->localizer->msg( $nsMsgKey )->isDisabled() ) {
			return $this->localizer->msg( $nsMsgKey )->parse();
		}

		return $this->getIslamTagline( 'islam-tagline' );
	}
	...
	private function getIslamTagline( string $msgKey ): string {
