...
/**
 * IslamComponentStickyHeader component
 */
class IslamComponentStickyHeader implements IslamComponent {

	private const SHARE_ICON = [
		'id' => 'islam-share-sticky-header',
		'clickTarget' => '#islam-share',
	...
	private function getIconButtons(): array {
		...
		foreach ( $icons as $icon ) {
			$button = new IslamComponentButton(
				"",
				$icon[ 'icon' ],
				$icon[ 'id' ],
				$icon[ 'class' ] ?? '',
				[
					'tabindex' => '-1',
					'data-mw-islam-click-target' => $icon[ 'clickTarget' ] ?? null,
				],
				'quiet',
				'default',
				'large',
