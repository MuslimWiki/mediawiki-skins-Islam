...
/**
 * IslamComponentSiteStats component
 * FIXME: Need unit test
 */
class IslamComponentSiteStats implements IslamComponent {
	...
	public function getTemplateData(): array {
		$config = $this->config;
		if ( !$config->get( 'IslamEnableDrawerSiteStats' ) ) {
			return [];
		}
		...
		if ( $config->get( 'IslamUseNumberFormatter' ) && class_exists( NumberFormatter::class ) ) {
			...
			try {
				 ...
				 * This should allow Islam to run as usual even if such exception is encountered.
				 */
			}
		}
		...
		foreach ( self::SITESTATS_ICON_MAP as $key => $icon ) {
			$items[] = [
				...
				'label' => $this->localizer->msg( "islam-sitestats-$key-label" )->text(),
			];
		}
