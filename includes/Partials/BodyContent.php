...
final class BodyContent extends Partial {
	 ...
	 */
	public const SECTION_CLASS = 'islam-section';
	...
	private function shouldFormatPage( Title $title ): bool {
		$shouldFormat = (
			$this->getConfigValue( 'IslamEnableCollapsibleSections' ) === true &&
			$title->canExist() &&
			!$title->isMainPage() &&
			$title->isContentPage() &&
	...
	private function prepareHeading( DOMDocument $doc, DOMElement $heading ): void {
		...
		$heading->setAttribute( 'class', $className . 'islam-section-heading' );
		...
		$indicator->setAttribute( 'class', 'islam-section-indicator islam-ui-icon mw-ui-icon-wikimedia-collapse' );
	...
	private function createSectionBodyElement( DOMDocument $doc, int $sectionNumber ): DOMElement {
		...
		$sectionBody->setAttribute( 'id', 'islam-section-' . $sectionNumber );
