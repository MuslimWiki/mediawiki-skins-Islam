	...
	SkinPageReadyConfigHook
{
	...
	public function onBeforePageDisplay( $out, $skin ): void {
		...
		if ( $this->getConfigValue( 'IslamEnablePreferences', $out ) === true ) {
			$script = file_get_contents( MW_INSTALL_PATH . '/skins/Islam/resources/skins.islam.scripts/inline.js' );
			$script = Html::inlineScript( $script );
			$script = RL\ResourceLoader::filter( 'minify-js', $script );
			$out->addHeadItem( 'skin.islam.inline', $script );
		}
	}
	...
	public function onOutputPageAfterGetHeadLinksArray( &$tags, $out ): void {
		if ( $out->getSkin()->getSkinName() !== 'islam' ) {
			return;
		}
	...
	public function onSkinBuildSidebar( $skin, &$bar ): void {
		...
		$out = $skin->getOutput();
		$globalToolsId = $this->getConfigValue( 'IslamGlobalToolsPortlet', $out );
	...
	public static function onSkinTemplateNavigation( $sktemplate, &$links ): void {
		// Be extra safe because it might be active on other skins with caching
		if ( $sktemplate->getSkinName() !== 'islam' ) {
			return;
		}
		...
		foreach ( $links['notifications'] as &$item ) {
			...
			if ( $icon ) {
				...
				$newLinkClass = [
					...
					'islam-echo-notification-badge',
					'islam-header__button',
	...
	private static function updateViewsMenu( array &$links ): void {
		...
		if ( isset( $links['views']['ve-edit'] ) && isset( $links['views']['edit'] ) ) {
			...
			self::appendClassToItem( $links['views']['ve-edit']['class'], [ 'islam-ve-edit-merged' ] );
			self::appendClassToItem( $links['views']['edit']['class'], [ 'islam-ve-edit-merged' ] );
	...
	private static function addIconsToMenuItems( array &$links, string $menu ): void {
		...
		foreach ( $links[$menu] as $key => $item ) {
			...
			if ( $icon ) {
				...
				$links[$menu][$key]['link-html'] = '<span class="islam-ui-icon mw-ui-icon-' . $icon . ' mw-ui-icon-wikimedia-' . $icon . '"></span>';
			}
		}
	}
