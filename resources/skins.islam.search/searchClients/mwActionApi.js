...
function adaptApiResponse( config, query, response, showDescription ) {
	...
	const getDescription = ( page ) => {
		switch ( config.wgIslamSearchDescriptionSource ) {
			case 'wikidata':
...
function mwActionApiSearchClient( config ) {
	return {
		 ...
		 * @type {fetchByTitle}
		 */
		fetchByTitle: ( q, limit = config.wgIslamMaxSearchResults, showDescription = true ) => {
			const cacheExpiry = config.wgSearchSuggestCacheExpiry;
			const descriptionSource = config.wgIslamSearchDescriptionSource;
