...
function searchHistory( config ) {
	return {
		data: [],
		limit: config.wgIslamMaxSearchResults,
		...
		set: function ( arr ) {
			if ( arr && this.isValidSearchHistory( arr ) ) {
				this.data = arr;
				mw.storage.set( 'skin-islam-search-history', JSON.stringify( this.data ) );
			}
		},
		add: function ( query ) {
		...
		clear: function () {
			this.set( [] );
			mw.storage.remove( 'skin-islam-search-history' );
		},
		init: function () {
			const storedData = mw.storage.get( 'skin-islam-search-history' );
			if ( storedData ) {
				try {
					this.data = JSON.parse( storedData );
				} catch ( error ) {
					mw.log.error( `[Islam] Error parsing search history. Stored data: ${ storedData }`, error );
				}
			}
		}
	};
