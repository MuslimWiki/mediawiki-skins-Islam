const SEARCH_LOADING_CLASS = 'islam-loading';

// Config object from getIslamSearchResourceLoaderConfig()
...
const typeahead = {
	...
	input: {
		...
		init: function ( inputEl ) {
			...
			wrapper.classList.add( 'islam-typeahead-input-group' );
			typeaheadInputElement.parentNode.insertBefore( wrapper, typeaheadInputElement );

			const overlay = document.createElement( 'div' );
			overlay.classList.add( 'islam-typeahead-input-overlay' );
			this.displayElement = document.createElement( 'span' );
			this.displayElement.textContent = typeaheadInputElement.value;
			this.displayElement.classList.add( 'islam-typeahead-input-overlay-query' );
			overlay.append( this.displayElement );

			typeaheadInputElement.classList.add( 'islam-typeahead-input' );
		...
		onFocus: function () {
			...
			typeahead.form.element.parentElement.classList.add( 'islam-search__card--expanded' );
	...
	items: {
		...
		/**
		 * Sets 'islam-typeahead__item--active' class on the element
		 *
		...
		set: function () {
			...
			this.elements = typeaheadElement.querySelectorAll( '.islam-typeahead-group[data-mw-typeahead-keyboard-navigation] .islam-typeahead-list-item-link' );
	...
	onBlur: function ( event ) {
		...
		if ( !typeaheadElement.contains( event.relatedTarget ) ) {
			...
			setTimeout( () => {
				typeahead.form.element.parentElement.classList.remove( 'islam-search__card--expanded' );
	...
	onClick: function ( event ) {
		...
		if ( typeahead.element.contains( event.target ) ) {
			const link = event.target.closest( '.islam-typeahead-list-item-link' );
			// Early exit if target is not a link
			if ( !link ) {
				return;
			}

			const group = event.target.closest( '.islam-typeahead-group' );
			...
			if ( historyType ) {
				...
				if ( historyType === 'query' ) {
					...
					const historyTextEl = link.querySelector( `.islam-typeahead-list-item-${ historyType }` );
	...
	updateSearchClient: function () {
		const typeaheadInputElement = typeahead.input.element;
		searchClient.setActive( config.wgIslamSearchGateway );
	...
	init: function ( formEl, inputEl ) {
		...
		const data = {
			...
			'array-lists': [
				{ type: 'action', class: 'islam-typeahead-group--chips', hidden: true, historyValue: 'query' },
...
async function getSuggestions() {
	...
	const renderSuggestions = ( results ) => {
		const groupEl = document.getElementById( 'islam-typeahead-group-page' );
		const listEl = document.getElementById( 'islam-typeahead-list-page' );
