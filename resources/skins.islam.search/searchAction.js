		...
		render: function ( searchQuery, templates ) {
			...
			items.push( {
				...
				text: mw.message( config.isAdvancedSearchExtensionEnabled ?
					'islam-search-advancedsearch' :
					'islam-search-fulltext'
				)
			} );
			...
			if ( config.isMediaSearchExtensionEnabled ) {
				...
				items.push( {
					...
					icon: 'imageGallery',
					text: mw.message( 'islam-search-mediasearch' )
				} );
			}
			...
			*/
			...
			if ( this.userCanEdit ) {
				...
				items.push( {
					...
					icon: 'edit',
					text: mw.message( 'islam-search-editpage' )
				} );
			}
			...
			document.getElementById( 'islam-typeahead-list-action' ).outerHTML = templates.TypeaheadList.render( data, partials ).html();
			document.getElementById( 'islam-typeahead-group-action' ).hidden = false;
		},
		clear: function () {
			document.getElementById( 'islam-typeahead-list-action' ).innerHTML = '';
			document.getElementById( 'islam-typeahead-group-action' ).hidden = true;
		},
		init: function () {
