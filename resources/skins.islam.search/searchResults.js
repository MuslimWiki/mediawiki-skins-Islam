...
/**
 * Returns an object with methods for handling search results in a islam search context.
	...
	return {
		getRedirectLabel: function ( title, matchedTitle, queryValue ) {
			...
			const generateRedirectHtml = () => {
				...
				div.classList.add( 'islam-typeahead__labelItem' );
				div.title = getRedirectMessage();

				const spanIcon = document.createElement( 'span' );
				spanIcon.classList.add( 'islam-ui-icon', 'mw-ui-icon-wikimedia-articleRedirect' );
		...
		highlightTitle: function ( title, match ) {
			...
			return title.replace( regex, '<span class="islam-typeahead__highlight">$&</span>' );
		},
		getPlaceholderHTML: function ( queryValue, templates ) {
			const data = {
				icon: 'articleNotFound',
				title: mw.message( 'islam-search-noresults-title', queryValue ).text(),
				description: mw.message( 'islam-search-noresults-desc' ).text()
			};
		...
		getResultsHTML: function ( results, queryValue, templates ) {
			...
			results.forEach( ( result, index ) => {
				...
				if ( result.thumbnail && result.thumbnail.url ) {
					...
					item.image.class = 'islam-typeahead-list-item-image--placeholder islam-ui-icon mw-ui-icon-wikimedia-image';
				}
				items.push( item );
			} );
		...
		clear: function () {
			...
			document.getElementById( 'islam-typeahead-list-page' ).innerHTML = '';
			document.getElementById( 'islam-typeahead-group-page' ).hidden = true;
			searchAction.clear();
		},
		init: function () {
			searchAction.init();
