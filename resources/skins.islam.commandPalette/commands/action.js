...
function fetchMenuItems() {
	...
	TARGET_PORTLET_IDS.forEach( ( portletId ) => {
		...
		const menuLabel = portletElement.querySelector( '.islam-menu__heading' )?.textContent?.trim();
		const links = portletElement.querySelectorAll( '.islam-menu__content-list li a' );
...
module.exports = {
	id: 'action',
	triggers: [ '/action:', '>' ],
	label: mw.message( 'islam-command-palette-command-action-label' ).text(),
	description: mw.message( 'islam-command-palette-command-action-description' ).text(),
