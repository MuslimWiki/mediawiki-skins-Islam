...
const { IslamCommandPaletteSearchClient, CommandPaletteSearchResponse, AbortableSearchFetch } = require( '../types.js' );
...
/**
 * @class
 * @implements {IslamCommandPaletteSearchClient}
 */
	...
	adaptApiResponse( query, response, showDescription ) {
		return {
			...
			results: response.pages.map( ( page ) => {
				...
				return {
					id: `islam-command-palette-item-page-${ page.id }`,
					type: 'page',
					label: page.title,
