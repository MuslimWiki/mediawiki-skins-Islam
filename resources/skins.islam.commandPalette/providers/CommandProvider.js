...
function registerCommand( handler ) {
	if ( typeof handler !== 'object' || handler === null ) {
		mw.log.warn( '[skins.islam.commandPalette|CommandProvider] Invalid handler provided for registration: not an object.' );
		return false;
	}
	const commandId = handler.id;
	if ( !commandId || typeof commandId !== 'string' || commandId.trim() === '' ) {
		mw.log.warn( `[skins.islam.commandPalette|CommandProvider] Invalid or missing command ID in handler: ${ commandId }` );
		return false;
	}

	if ( commandRegistry.has( commandId ) ) {
		mw.log.warn( `[skins.islam.commandPalette|CommandProvider] Command "${ commandId }" is already registered. Overwriting.` );
	}
...
function getCommandListItems( filterPrefix ) {
	...
	try {
		return entries.map( ( [ id, handler ] ) => ( {
			id: `islam-command-palette-item-command-${ id }`,
			type: 'command',
		...
		mw.log.error( '[skins.islam.commandPalette|CommandProvider|getCommandListItems] Error during mapping:', error );
...
mw.hook( 'skins.islam.commandPalette.registerCommand' ).fire( { registerCommand } );
...
function findMatchingCommand( query ) {
	...
	if ( !handler ) {
		mw.log.warn( `[skins.islam.commandPalette|CommandProvider] Handler for command "${ id }" (matched via trigger "${ trigger }") not found in registry. This indicates an inconsistency.` );
		return null;
	}
...
async function executeSubQueryCommand( handler, query, trigger, commandId ) {
	...
	try {
		...
		mw.log.error( `[skins.islam.commandPalette|CommandProvider] Handler getResults for "${ commandId }" failed:`, err );
		return [];
	}
}
...
module.exports = {
	...
	async onResultSelect( item ) {
		...
		if ( sourceParts?.[ 0 ] !== 'command' || sourceParts.length < 2 ) {
			mw.log.warn( `[skins.islam.commandPalette|CommandProvider] Invalid source format for item: ${ item.source }` );
			return { action: 'none' };
		}
		...
		if ( !handler ) {
			mw.log.warn( `[skins.islam.commandPalette|CommandProvider] Could not find handler for source "${ item.source }". Item:`, item );
			return { action: 'none' };
		}
		...
		try {
			...
			mw.log.error( `[skins.islam.commandPalette|CommandProvider] Error during selection delegation for command handler "${ handlerId }":`, err, 'Item:', item );
			return { action: 'none' };
		}
	}
};
