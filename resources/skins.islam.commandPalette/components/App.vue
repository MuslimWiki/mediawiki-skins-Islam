<template>
	<div
		v-if="isOpen"
		class="islam-command-palette-backdrop"
		@click="close"></div>
	<div
		v-if="isOpen"
		class="islam-command-palette"
	...
	>
		...
		<div
			...
			class="islam-command-palette__results"
		>
			...
			<template v-else-if="searchStore.searchQuery && !searchStore.isPending && displayedItems.length === 0">
				<command-palette-empty-state
					:title="$i18n( 'islam-search-noresults-title' ).params( [ searchStore.searchQuery ] ).text()"
...
module.exports = exports = defineComponent( {
	...
	setup() {
		...
		const handleRootKeydown = ( event ) => {
			...
			const isActionFocused = event.target?.closest( '.islam-command-palette-list-item__action' );
...
.islam-command-palette {
	--islam-command-palette-side-padding: var( --space-md );
	position: fixed;
	top: var( --space-xs );
	right: var( --space-xs );
	left: var( --space-xs );
	...
	.mixin-islam-frosted-glass-simple;
	...
	&__no-results {
		padding: var( --space-md ) var( --islam-command-palette-side-padding );
		text-align: center;
	}
}
</style>
