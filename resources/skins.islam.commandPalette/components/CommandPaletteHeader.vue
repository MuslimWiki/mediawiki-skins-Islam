<template>
	<div class="islam-command-palette__search">
		<cdx-text-input
			ref="searchInputRef"
			:model-value="value"
			class="islam-command-palette__input"
			input-type="search"
		...
		<div
			...
			class="islam-command-palette__progress-indicator islam-loading"
		></div>
	</div>
</template>
...
const { CdxTextInput } = mw.loader.require( 'skins.islam.commandPalette.codex' );
...
.islam-command-palette {
	&__search {
		...
		padding: var( --space-sm ) calc( var( --islam-command-palette-side-padding ) - @spacing-50 );
