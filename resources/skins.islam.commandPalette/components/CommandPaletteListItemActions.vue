<template>
	<div
		...
		class="islam-command-palette-list-item__actions"
		:class="{ 'islam-command-palette-list-item__actions--visible': highlighted }"
	>
		<cdx-button
			v-for="action in actions"
			:key="action.id"
			:ref="( el ) => setButtonRef( el, action.id )"
			class="islam-command-palette-list-item__action"
		...
		>
			<cdx-icon
				:icon="action.icon"
				size="small"
				class="islam-command-palette-list-item__action__icon"
			></cdx-icon>
		</cdx-button>
	</div>
</template>
...
const { CdxIcon, CdxButton } = mw.loader.require( 'skins.islam.commandPalette.codex' );
...
.islam-command-palette-list-item {
	&__actions {
		position: absolute;
		inset-inline-end: var( --islam-command-palette-side-padding );
		top: 0;
		display: flex;
		gap: var( --space-xxs );
		align-items: center;
