...
const RECENT_ITEMS_KEY = 'skin-islam-command-palette-recent-items';
...
function createRecentItems() {
	...
	function saveSearchQuery( query, searchUrl ) {
		saveRecentItem( {
			...
			id: `islam-command-palette-result-search-${ mw.util.escapeIdForAttribute( query ) }`,
			label: query,
			url: searchUrl,
	...
	function getRecentItems() {
		...
		const dismissAction = {
			id: 'dismiss',
			label: mw.msg( 'islam-command-palette-dismiss' ),
			icon: cdxIconTrash
		};
