...
/**
 * Wrapper for mw.message to replace some message keys with islam-specific messages
 * * Some message keys are not merged into core upstream
 * * Translatewiki does not allow for duplicated message keys
 * * @see https://github.com/StarIslamTools/mediawiki-skins-Islam/commit/a741639085d70c22a9f49890542a142a223bf981#r141359264
 *
...
function getMessage( messageKey ) {
	if ( messageKey.startsWith( 'skin-theme-' ) ) {
		messageKey = messageKey.replace( 'skin-theme-', 'islam-theme-' );
	}
...
function appendRadioToggle( parent, featureName, value, currentValue, config ) {
	...
	input.classList.add( 'islam-client-prefs-radio__input' );
	...
	icon.classList.add( 'islam-client-prefs-radio__icon' );
	const label = makeLabelElement( featureName, value );
	// label.classList.add( 'cdx-radio__label' );
	label.classList.add( 'islam-client-prefs-radio__label' );
	const container = document.createElement( 'div' );
	// container.classList.add( 'cdx-radio' );
	container.classList.add( 'islam-client-prefs-radio' );
...
function appendToggleSwitch( form, featureName, labelElement, currentValue, config ) {
	...
	input.classList.add( 'islam-client-prefs-toggle-switch__input' );
	const switcher = document.createElement( 'span' );
	// switcher.classList.add( 'cdx-toggle-switch__switch' );
	switcher.classList.add( 'islam-client-prefs-toggle-switch__switch' );
	const grip = document.createElement( 'span' );
	// grip.classList.add( 'cdx-toggle-switch__switch__grip' );
	grip.classList.add( 'islam-client-prefs-toggle-switch__switch__grip' );
	...
	label.classList.add( 'islam-client-prefs-toggle-switch__label' );
	const toggleSwitch = document.createElement( 'span' );
	// toggleSwitch.classList.add( 'cdx-toggle-switch' );
	toggleSwitch.classList.add( 'islam-client-prefs-toggle-switch' );
