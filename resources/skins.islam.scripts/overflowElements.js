...
class OverflowElement {
	constructor( element, isPointerDevice ) {
		...
		this.headerToSticky = this.element.querySelector( '.islam-overflow-sticky-header' );
	}
	...
	updateState() {
		...
		window.requestAnimationFrame( () => {
			const updateClasses = [
				[ isLeftOverflowing, 'islam-overflow--left' ],
				[ isRightOverflowing, 'islam-overflow--right' ]
			];
			this.toggleClasses( updateClasses );
			if ( this.stickyHeader ) {
				this.stickyHeader.style.setProperty( '--islam-overflow-scroll-x', this.contentScrollLeft + 'px' );
			}
		} );
	}
	...
	handleInheritedClasses() {
		const inheritedClasses = config.wgIslamOverflowInheritedClasses;
	...
	/**
	 * Wraps the element in a div container with the class 'islam-overflow-wrapper'.
	 ...
	 * Creates a new div wrapper element, adds the class 'islam-overflow-wrapper', and appends it to the parent node before the element.
	...
	wrap() {
		if ( !this.element || !this.element.parentNode ) {
			mw.log.error(
				'[Islam] Element or element.parentNode is null or undefined. Please check if the element or element.parentNode is null or undefined.'
			);
			return;
		}
		try {
			...
			wrapper.className = 'islam-overflow-wrapper';
			...
			if ( this.isPointerDevice ) {
				const createButton = ( type ) => {
					...
					button.className = `islam-overflow-navButton islam-overflow-navButton-${ type } islam-ui-icon mw-ui-icon-wikimedia-collapse`;
				...
				nav.className = 'islam-overflow-nav';
			...
			content.className = 'islam-overflow-content';
			...
			mw.log.error(
				`[Islam] Error occurred while wrapping element: ${ error.message }`
			);
		}
	}
	...
	setupStickyHeader() {
		...
		this.stickyHeader.classList.add( 'islam-overflow-content-sticky-header' );
	...
	onClick( event ) {
		...
		if ( !target.classList.contains( 'islam-overflow-navButton' ) ) {
			return;
		}
		// Prevent triggering the form submit action (e.g. realtime preview in WikiEditor)
		event.preventDefault();
		const offset = this.wrapper.offsetWidth / 2;
		if ( target.classList.contains( 'islam-overflow-navButton-left' ) ) {
			this.scrollContent( -offset );
		} else if (
			target.classList.contains( 'islam-overflow-navButton-right' )
		) {
			this.scrollContent( offset );
		}
	}
...
function init( bodyContent ) {
	const nowrapClasses = config.wgIslamOverflowNowrapClasses;
	if ( !nowrapClasses || !Array.isArray( nowrapClasses ) ) {
		mw.log.error(
			'[Islam] Invalid or missing $wgIslamOverflowNowrapClasses. Cannot proceed with wrapping element.'
		);
		return;
	}

	const overflowElements = bodyContent.querySelectorAll(
		'.islam-overflow, .wikitable:not( .wikitable .wikitable )'
	);
