...
function renderSearchClearButton( input ) {
	...
	clearButton.classList.add( 'islam-search__clear', 'islam-search__formButton' );
	// TODO: Add i18n for the message below
	// clearButton.setAttribute( 'aria-label', 'Clear search input' );
	clearIcon.classList.add( 'islam-ui-icon', 'mw-ui-icon-wikimedia-trash' );
...
function bindSearchTrigger( details ) {
	document.querySelectorAll( '.islam-search-trigger' ).forEach( ( trigger ) => {
...
function openSearch( details ) {
	if ( config.wgIslamEnableCommandPalette ) {
		details.click();
	} else {
		details.open = true;
	}
...
/**
 * Initializes the search functionality for the Islam search boxes.
 *
 * @param {Window} window
 * @return {void}
 */
function initSearch( window ) {
	...
	if ( config.wgIslamEnableCommandPalette ) {
		...
		mw.loader.load( 'skins.islam.commandPalette' );
		return;
	}

	const searchBoxes = document.querySelectorAll( '.islam-search-box' );
	...
	searchBoxes.forEach( ( searchBox ) => {
		...
		loadSearchModule( input, config.wgIslamSearchModule, () => {
