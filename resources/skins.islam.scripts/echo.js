...
function init() {
	...
	const alertIcon = notifications.querySelector( '#pt-notifications-alert > a > .islam-ui-icon' ).cloneNode();
	const noticeIcon = notifications.querySelector( '#pt-notifications-notice > a > .islam-ui-icon' ).cloneNode();
	...
	const setupFosterHome = ( badge, anchor ) => {
		...
		anchor.classList.add( 'islam-header__button', 'islam-echo-notification-badge' );
		...
		for ( const mutation of mutations ) {
			if ( mutation.type === 'childList' ) {
				...
				for ( const removedNode of removedNodes ) {
					if ( !( removedNode instanceof HTMLSpanElement ) || !removedNode.classList.contains( 'islam-ui-icon' ) ) {
						return;
					}
